...
public class CassandraClient2 {
	...
	private static final String SUPER_COLUMN = "Super1";
	 ...
	 * @param _port
	 */
	public CassandraClient2(String _host, int _port) {
		//ホスト名の指定
		host = _host;
		//ポート番号の指定
		port = _port;
	 ...
	 */
	private static Cassandra.Client openConnection() {
		try {
	...
	public void insertDocFreq(List<Map<String, String>> data) {
		try {
			...
			for (int i = 0; i < data.size(); i++) {
				...
				mutationList.add(toMutation(id, doc, timestamp));
	...
	public void insertURL(String url, String id) {
		try {
			...
			columnPath.setColumn(url.getBytes("utf-8"));
			//レコードを挿入
			client.insert(KEYSPACE, "URLs", columnPath, id.getBytes("utf-8"), System.currentTimeMillis(), ConsistencyLevel.QUORUM);
		} catch(Exception e) {
			e.printStackTrace();
		}
	}
	...
	public List<Map<String, String>> getURLs() {
		try {
			...
			List<ColumnOrSuperColumn> results = client.get_slice(KEYSPACE, "URLs", columnParent, slicePredicate, ConsistencyLevel.QUORUM);
			//結果を出力する変数
			List<Map<String, String>> list = new ArrayList<Map<String, String>>();
